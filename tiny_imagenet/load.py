...
x_ = Input(shape=(56, 56, N_channel))
...
x = Conv2D(64, kernel_size=(3, 3), strides=(1, 1),  use_bias=False, kernel_regularizer=regularizers.l2(l2))(x_)
x = BatchNormalization()(x)
x = Activation('relu')(x)
...
#x = Activation('relu')(x)
#x = tf.keras.layers.Dropout(0.3)(x)
x = Dense(200,  kernel_initializer='he_normal')(x) #multi-class
#x = BatchNormalization()(x)
...
def lr_schedule(epoch):
    learning_rate = 1e-1
    if epoch+l_epoch < 55:
        return learning_rate * (0.2 ** (epoch+l_epoch // 40))
    if ((epoch+l_epoch >= 55) and (epoch+l_epoch < 65)):
        return learning_rate * (0.04 ** (epoch+l_epoch // 55))
    
    if epoch >= 65:
        return learning_rate * (0.008 ** (epoch+l_epoch // 65))
...
model = tf.keras.models.load_model('weights.38-0.4596.hdf5')
...
history = model.fit_generator(datagen.flow(random_crop(x_train), y_train,
                                         batch_size=128),
                            steps_per_epoch=x_train.shape[0] // 128,
                            epochs=70,
                            validation_data=(central_crop(x_test), y_test),callbacks=callbacks,verbose=1)
