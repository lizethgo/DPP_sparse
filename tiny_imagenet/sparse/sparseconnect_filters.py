...
class DPS_topk(Layer):
    ...
    def call(self, inp):
        ...
        if self.is_CNN == True:
            ...
            softSamples = tf.nn.sigmoid((tf.math.log(tf.multiply(prob_exp,1-cumMask+1e-7))+tf.tile(tf.expand_dims(GN,-2),(1,1,self.k,1)))/Temp)  #[BS,output_nodes,k,input_nodes]
...
class sparseconnect_CNN_layer(Layer): 
    ...
    def call(self, x):
        ...
        y = tf.nn.conv2d(x, self.k, self.strides, padding = 'SAME')
...
class sparseconnect_mask(Layer): 
    """
    Sparse convolutional layer (filter level)
    - Generates trainable logits (D_ch)
    - Call DPS_topK to perform optimization
    ...
    def __init__(self,
        ...
        super(sparseconnect_mask, self).__init__(name=name, **kwargs) 
    ...
    def build(self, input_shape): 
        ...
        super(sparseconnect_mask, self).build(input_shape) 
    ...
    def call(self, x):
        ...
        if self.one_per_batch:
            ...
            print('AC shape', A_ch.shape)
            y=tf.multiply(A_ch, x)
            #y = tf.nn.bias_add(y,self.b)  
