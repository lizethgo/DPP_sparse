...
def res_identity_exp(x, filters, stride): 
  ...
  x_skip = Conv2D(f1, kernel_size=(1, 1), strides=(stride, stride), padding='same',  use_bias=False, kernel_initializer="he_normal", kernel_regularizer=tf.keras.regularizers.l2(0.0001))(x_skip)
...
def res_identity_sparse(x, filters, channel_in, stride, ratio): 
  ...
  x = Conv2D(f1, kernel_size=(3, 3), strides=(stride, stride), padding='same', kernel_regularizer=regularizers.l2(0.0001))(x)
  ...
  x = sparseconnect_mask(n_connect_ch = round(ratio*f1), 
                          filters = f1, 
                          n_epochs=250, 
                          tempIncr=5)(x)
  x = Conv2D(f2, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_regularizer=regularizers.l2(0.0001))(x)
  x = BatchNormalization()(x)
...
def res_identity_exp_sparse(x, filters, channel_in, stride, ratio): 
  ...
  #first block 
  # bottleneck (but size kept same with padding)
  x = sparseconnect_mask(n_connect_ch = round(ratio*channel_in), 
                          filters = channel_in, 
                          n_epochs=250, 
                          tempIncr=5)(x)
                              
  x = Conv2D(f1, kernel_size=(3, 3), strides=(stride, stride), padding='same', kernel_regularizer=regularizers.l2(0.0001))(x)
  # x = sparseconnect_CNN_layer_filter(n_connect_ch = round(ratio*channel_in), 
                              # filters = f1, 
                              # kernel_size = 3,
                              # padding = 'SAME',
                              # channel_size = channel_in, 
  ...
  # x_skip = sparseconnect_CNN_layer_filter(n_connect_ch = round(ratio*f1), 
                              ...
                              # n_epochs=250, 
                              # tempIncr=5)(x)
  x_skip = Conv2D(f2, kernel_size=(1, 1), strides=(stride,stride), padding='same', kernel_regularizer=regularizers.l2(0.0001))(x_skip)
  x_skip = BatchNormalization()(x_skip)
...
x = Conv2D(64, kernel_size=(3, 3), strides=(1, 1),  use_bias=False)(x_)
# x = sparseconnect_CNN_layer_filter(n_connect_ch = 64, 
                              # filters = 64, 
                              # kernel_size = 3,
                              # padding = 'SAME',
                              # channel_size = 3, 
...
#x = res_identity_exp_sparse(x, filters=(64, 64), channel_in = 64, stride=1, ratio = 0.6)
x = res_identity(x, filters=(64, 64), stride=1)


# 3rd stage
#x = res_identity_exp_sparse(x, filters=(128, 128), channel_in = 64, stride=2, ratio = 0.3)
...
# 4th stage
x = res_identity_exp_sparse(x, filters=(256, 256), channel_in = 128, stride=2,  ratio = 0.25)
#x = res_identity_exp(x, filters=(256, 256), stride=2)
#x = res_identity(x, filters=(256, 256), stride=1)
x = res_identity_sparse(x, filters=(256, 256), channel_in = 256, stride=1,  ratio = 0.25)


# 5th stage
#x = res_identity_exp(x, filters=(512, 512), stride=2)
x = res_identity_exp_sparse(x, filters=(512, 512), channel_in = 256, stride=2,  ratio = 0.25)
x = res_identity_sparse(x, filters=(512, 512), channel_in = 512, stride=1,  ratio = 0.25)
