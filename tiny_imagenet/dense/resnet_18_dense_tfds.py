...
#x = Activation('relu')(x)
#x = tf.keras.layers.Dropout(0.3)(x)
y = Dense(200, activation='softmax', kernel_initializer='he_normal')(x) #multi-class
...
def lr_schedule(epoch):
    learning_rate = 1e-1
    if epoch < 50:
        return learning_rate * (0.1 ** (epoch // 40))
    if ((epoch >= 50) and (epoch < 200)):
        return learning_rate * (0.01 ** (epoch // 50))
    
    if epoch >= 60:
        return learning_rate * (0.001 ** (epoch // 60))
...
#
#
#history = model.fit(x=x_train, y=y_train, batch_size=128, epochs=250, verbose=1,
#          callbacks = callbacks,
#          validation_data=(x_test,y_test)) 
import h5py
            
file = h5py.File('resnet_cifa120.h5py','w')
