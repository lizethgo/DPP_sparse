...
path = 'G:\\Liz Gonzalez Carabin\\datasets\\tiny-imagenet-200\\tiny-imagenet-200\\'
...
def res_identity(x, filters, stride): 
  ...
  x = Activation('relu')(x)
  x = tf.keras.layers.Dropout(0.13)(x)
  #x = tf.keras.layers.Dropout(0.3)(x)
...
def lr_schedule(epoch):
    ...
    if ((epoch >= 55) and (epoch < 65)):
        return learning_rate * (0.01 ** (epoch // 55))
    
    if epoch >= 65:
        return learning_rate * (0.001 ** (epoch // 65))
...
datagen = ImageDataGenerator(
    ...
    rotation_range=1,  # randomly rotate images in the range (degrees, 0 to 180) 1
    width_shift_range=0,  # randomly shift images horizontally (fraction of total width)
    height_shift_range=0.15,  # randomly shift images vertically (fraction of total height) 0.15
